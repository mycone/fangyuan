...
class Helper
{
    ...
    protected static function getDefaultRoutes()
    {
        if (self::$_defaultRoutes === null) {
            ...
            if ($cache && ($routes = $cache->get($roles)) !== false) {
                ...
                if ($cache) {
                    $cache->set($roles, self::$_defaultRoutes, MenuHelper::CACHE_DURATION, new TagDependency([
                        'tags' => MenuHelper::CACHE_TAG
                    ]));
                }
            }
        }
    ...
    public static function getRoutesByUser($userId)
    {
        if (!isset(self::$_userRoutes[$userId])) {
            ...
            if ($cache && ($routes = $cache->get([__METHOD__, $userId])) !== false) {
                ...
                self::$_userRoutes[$userId] = $routes;
                if ($cache) {
                    $cache->set([__METHOD__, $userId], $routes, MenuHelper::CACHE_DURATION, new TagDependency([
                        'tags' => MenuHelper::CACHE_TAG
                    ]));
                }
            }
    ...
    public static function invalidate()
    {
        if (Yii::$app->cache !== null) {
            TagDependency::invalidate(Yii::$app->cache, MenuHelper::CACHE_TAG);
        }
    }
}
