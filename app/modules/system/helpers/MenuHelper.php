    ...
    public static function getAssignedMenu($userId, $root = null, $callback = null, $refresh = false)
    {
        ...
        if ($refresh || $cache === null || ($assigned = $cache->get($key)) === false) {
            ...
            */
            ...
            $routes = Yii::$app->rbacService->getPermissionsRoutesByUser($userId);
            $prefix = '\\';
            ...
            if ($cache !== null) {
                $cache->set($key, $assigned, self::CACHE_DURATION, new TagDependency([
                    'tags' => self::CACHE_TAG
                ]));
            }
        }
        ...
        foreach ($assigned as $id) {
            ...
            if ($menu['parent'] == $parent) {
                ...
                if ($callback !== null) {
                    ...
                    $icon = isset($data['icon']) ? $data['icon'] : 'fa fa-circle-o';
                    $item = [
                        'label' => $menu['name'],
                        'icon' => $icon,
